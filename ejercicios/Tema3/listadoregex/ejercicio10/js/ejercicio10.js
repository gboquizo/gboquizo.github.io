 ...
 */
{
    ...
    let createPage = function () {
        ...
        h2.innerHTML = 'Listado regex en JS.' + newline + 'Ejercicio 10';
    ...
    let createDefinans = function (fragment) {
        ...
        firstParagraph.innerHTML =
            `<h4>Busca en el objeto String funciones que simulen la funcionalidad de exec()</h4>
            <p class="info">Podemos emplear el método match(), que busca una coincidencia ` +
            newline + `frente a una expresión regular y devuelve las coincidencias encontradas.</p>
            <ol class="info">
                <li>Podemos usar match() así:
                </li>
                    <p class = "info4">let prueba = "The Quick Brown Fox Jumps Over The Lazy Dog"; </p>
                    <p class = "info4">let resultado1 = prueba.match(/quick\s(brown).+?(jumps)/ig);;</p>
                    </p>
            </ol>
        `;
    ...
    let createExercise = function (fragment) {
        ...
        let resultado1 = prueba.match(/quick\s(brown).+?(jumps)/ig);

        paragraph.innerHTML = "Testeando el uso de match(): " + resultado1;
