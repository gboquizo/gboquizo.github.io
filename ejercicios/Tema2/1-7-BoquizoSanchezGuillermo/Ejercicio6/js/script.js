 ...
 */
{
  ...
  function validarDni() {
    ...
    let letterOfDNI = prompt("Introduce la letra de tu DNI").toUpperCase();
    let calculatedLetter = letters[numberOfDNI % 23];
    console.log("La letra que has proporcionado es " + letterOfDNI);
    console.log("La letra que corresponde a tu DNI es " + calculatedLetter);
    if (calculatedLetter != letterOfDNI) {
      alert("La letra o el número proporcionados no son correctos");
      return;
    }
    alert("El número de DNI y su letra son correctos, tu DNI es " +
      numberOfDNI + " y la letra es " + calculatedLetter
    );
  }
  validarDni();
}